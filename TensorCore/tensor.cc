...
#undef ADD_CODE
        ...
        std::pair<Tensor, Tensor> tensor_broadcasting(const Tensor& a, const Tensor& b, unsigned char begin_dim, unsigned char end_dim)
        {
            ...
            for (unsigned int i = max_dim - 1 - end_dim; i < max_dim && i >= begin_dim; i--)
                if (temp_a.get_buffer().shape().begin()[i] == temp_b.get_buffer().shape().begin()[i])
                    ...
                    throw std::exception("Cannot broadcasting");
        ...
        Tensor add_dim(const std::vector<Tensor>& list)
        {
            ...
            void* content = operator new(list.size() * list[0].get_buffer().data_size());
            ...
            for (unsigned int i = 0; i < list.size(); i++)
            {
                ...
                std::memcpy(temp_ptr, list[i].get_buffer().data(), list[i].get_buffer().data_size());
            }
        ...
        Tensor Tensor::slice(const std::initializer_list<Slice>& slice_arr, bool is_derive) const
        {
            ...
            if (is_derive)
            {
                ...
                temp.push_back(std::make_pair(*this, Derivation(values(std::initializer_list(derive_shape.begin(), derive_shape.begin() + slice_arr.size()), 0).tensor_cast(this->get_buffer().type()), derive_slice, false, slice_arr)));
            }
            const Slice slice_begin = this->correct_slice(slice_arr.begin()[0]);
            std::vector<Tensor> temp_tensors;
        ...
        Tensor Tensor::transpose(unsigned char dim0, unsigned char dim1, bool is_derive) const
        {
            ...
            temp_tensor = derive_transpose(temp_tensor.reshape(std::initializer_list(std::begin(tranpose_shape), std::end(tranpose_shape)), is_derive), Tensor(), is_derive, nullptr);
            if (dim1 - dim0 > 1)
            {
            ...
            a[dim0] = a[dim1];
            a[dim1] = temp1;
            return temp_tensor.reshape(std::initializer_list(a.begin()._Ptr, a.end()._Ptr), is_derive);
        }
        ...
        {
            return this->mean(std::initializer_list<unsigned char>(list_dims.begin()._Ptr, list_dims.end()._Ptr));
        }

        Tensor Tensor::variance(const std::vector<unsigned char>& list_dims) const
        {
            return this->variance(std::initializer_list<unsigned char>(list_dims.begin()._Ptr, list_dims.end()._Ptr));
        }
        ...
        {
            return this->reshape(std::initializer_list<unsigned int>(dim_sizes.begin()._Ptr, dim_sizes.end()._Ptr));
        }
...
#undef ADD_CODE
        ...
        void t_add(const Tensor& a, const std::initializer_list<unsigned int>& b_shape, std::vector<Tensor>& t_vec, std::vector<Tensor::Slice>& slice_vec, const unsigned int* strides)
        {
            const unsigned int& end = b_shape.begin()[slice_vec.size() + 1];
            if (&end == b_shape.end() - 1)
                t_vec.push_back(a[std::initializer_list(slice_vec.begin()._Ptr, slice_vec.end()._Ptr)]);
            else
            {
                std::initializer_list<unsigned int> a_shape = a.get_buffer().shape();
                unsigned int e = a_shape.begin()[slice_vec.size() + 2] - end;
                for (int i = 0; i < e; i+= strides[slice_vec.size()])
                {
                    slice_vec.push_back({ static_cast<int>(i), static_cast<int>(i + e), 1 });
                    t_add(a, b_shape, t_vec, slice_vec, strides);
                    slice_vec.pop_back();
                }
            }
        }

        Tensor cross_colerate(const Tensor& a, const Tensor& b, bool is_derive, const DataBuffer& dims_data)
        {
            std::initializer_list<unsigned int> a_shape = a.get_buffer().shape();
            std::initializer_list<unsigned int> b_shape = b.get_buffer().shape();
            assert(a_shape.size() == b_shape.size());
            const unsigned int* strides = static_cast<const unsigned int*>(dims_data.get_data());
            const unsigned int& batch_size = a.get_buffer().shape().begin()[0];
            unsigned int* result_size = new unsigned int[dims_data.get_data_size()/ sizeof(unsigned int)];
            std::vector<Derivation> temp;
            std::vector<Tensor> t_vec;
            unsigned int matmul_size[3]{1,1,1};
            t_add(a, b_shape, t_vec, std::vector<Tensor::Slice>{{},{}}, strides);
            Tensor ad = add_dim(t_vec);
            Tensor test = matmul(ad.reshape({ matmul_size[0], matmul_size[1] }, is_derive), b.reshape({ matmul_size[1], matmul_size[2] }, false), false, nullptr);
            delete[] result_size;
            return test;
        }
        ...
        {
            return tensor_rand(std::initializer_list<unsigned int>(shape_vector.begin()._Ptr, shape_vector.end()._Ptr), seed);
        }
