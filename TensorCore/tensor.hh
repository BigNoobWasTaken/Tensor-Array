...
namespace ai
{
	namespace value
	{
        ...
        public:
            ...
            friend void t_add(const Tensor&, const std::initializer_list<unsigned int>&, std::vector<Tensor>&, std::vector<Tensor::Slice>&, const unsigned int*);

            friend Tensor cross_colerate(const Tensor&, const Tensor&, bool, const DataBuffer&);
...
#undef ADD_CODE
        ...
        {
            return values(std::initializer_list<unsigned int>(shape_vector.begin()._Ptr, shape_vector.end()._Ptr), value);
        }
}
}
